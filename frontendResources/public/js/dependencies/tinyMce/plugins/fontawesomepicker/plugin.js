(function () {
	...
	var global = tinymce.PluginManager
	var jsonpCache = {}
		...
		return function (editor, pluginUrl) {
			...
			return {
				...
				openAfter: function () {
					var that = this
					that.changeNav('all')
					$('.mce-fontawesomepicker--nav__item', document)
						.off('click').on('click', function () {
							var title = this.getAttribute('title')
							that.changeNav(title)
						})

					$('.mce-fontawesomepicker--icon', document)
						.off('click').on('click', function () {
                            var iconEl = this.querySelector("span i")
                            iconEl.setAttribute("contenteditable", "false")
							that.insertIcon(iconEl)
						})
				},
				renderNav: function () {
					var html = ''
	...
	var SettingPanel = function (editor) {
		...
		var targetIcon = $(target).children('.mark-fapk')[0]
		var rotate = $(targetIcon).css('transform').replace(/(matrix|none|\(|\))/g, '')
		...
		editor.windowManager.open({
			...
			initialData: {
				'icon-style': $(target).attr('icon-style') || 'none',
				'icon-size': $(targetIcon).css('font-size'),
				'icon-color': $(targetIcon).css('color') || '',
				'icon-rotate': rotate + 'deg',
				'style-color': $(target).attr('style-color') || ''
			},
			onSubmit: function (api) {
				...
				if ( data['icon-style'] === 'none' ) {
					...
					$(target).attr('icon-style', data['icon-style'])
					$(target).attr('style-color', data['style-color'])
					style['display'] = 'inline-block'
