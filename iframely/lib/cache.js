    ...
    cache.withCache = function(key, func, options, callback) {
        ...
        if (options && options.multiCache) {
            cache_get = options.multiCache.registerGetCb.bind(options.multiCache);
            cache_set = options.multiCache.registerSet.bind(options.multiCache);
            cache_skip_get = options.multiCache.skipGet.bind(options.multiCache);
            cache_skip_set = options.multiCache.skipSet.bind(options.multiCache);
        } else {
            cache_get = cache.get;
            cache_set = cache.set;
        }
    ...
    * */
    ...
    MultiCache.prototype.runAllGets = function() {
        ...
        var getsCount = Object.keys(this.getCbDict).length;

        var keysList = Object.keys(this.keysDict).sort();
    ...
    MultiCache.prototype.registerSet = function(key, value, options) {
        ...
        // Use smallest ttl.
        if (options.ttl && this.options.ttl && options.ttl < this.options.ttl) {
    ...
    MultiCache.prototype._checkAllSet = function() {
        if (Object.keys(this.keysDict).length === Object.keys(this.setKeysDict).length && this.multiKey) {
            this._runAllSets();
        }
    };
