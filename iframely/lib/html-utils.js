    ...
    function wrapContainer($element, data, options) {
        ...
        if (!forceWidthLimitContainer && media) {
            ...
            if (media.height && !media.width && !media["aspect-ratio"] && !media["max-width"]) {
        ...
        if (media) {
            ...
            if (media["max-width"]) {
                ...
                $widthLimitContainer.css("max-width", maxWidth);
        ...
        "iframe": {
            ...
            generate: function(data, options) {
                ...
                if (options && options.title) {
                    $iframe.attr('title', options.title);
                }
                ...
                var allow = [];
                if ((options && options.autoplayMode)) {
                    allow.push('autoplay');
                }
                if (data.rel instanceof Array && CONFIG.FEATURES instanceof Array) {
                    if (data.rel.indexOf(CONFIG.R.player) > -1) {
                        allow.push('encrypted-media');
                    }
                    data.rel.forEach(function(rel) {
                        if (CONFIG.FEATURES.indexOf(rel) > -1 && allow.indexOf(rel) == -1) {
                            allow.push(rel);
                        }
                    })
                }
                if (allow.length > 0) {
                    $iframe.attr('allow', allow.join('; '));
                }
    ...
    function isNonHtml5Audio(l1, options) {
        if (options.mp4_first) {
            // Reverse logic.
            return l1.type.indexOf('audio/') === 0;
        } else {
            return l1.type.indexOf('audio/') !== 0;
        }
    }
    ...
    export function findMainLink(iframely_data, options) {
        ...
        if (iframely_data) {
            ...
            if (options && options.mediaPriority) {
