...
export default {
    ...
    prepareLink: function(url, link, options, iterationPluginContext, cb) {
        ...
        if (!link.href) {
            return cb();
        } else if (/^image/i.test(link.type) && /^data:/i.test(link.href)) {
            link.error = "Data images are not supported";
            return cb();
        }
        ...
        if (options.refresh) {
            doNotWaitFunctionIfNoCache = false;
        }
            ...
            function(error, data) {
                ...
                if (error) {
                    ...
                    if (/webp/i.test(data.format)
                        ...
                        link.error = "To turn on WebP images, configure images.webp=false in providerOptions";
                    }
